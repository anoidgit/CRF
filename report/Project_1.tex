...
We also tried to evaluate the objective function and error every 5, 30 and 100 updates of SGD/SAG-NUS, more frequent computation shows more fluctuation of object value, this also makes sense because of the randomness by sampling. We also see that different $\lambda$ takes different time to converge, generally large $\lambda$ should have a larger maximal iteration number.
...
\includegraphics[width=5cm]{t3_letter_err}}
~~
\subfigure[Word-wise error]{
\includegraphics[width=5cm]{t3_word_err}}
...
{\bf Solution T4.}
Figure \ref{fig:compare_word_err} gives the comparison of letter-wise and word-wise error of three methods, trained on the entire training set with 3348 words. In LBFGS, the solver has 184 effective passes, however, since the torch  version is quite slow, and both SGD and SAG-NUS tend to converge after 20 effective passes, we constrain the x axis to be $0\sim100$. Consistent with our conclusion above, SAG-NUS performs the worse in terms of test error and LBFGS achieves the lowest letter-wise and word-wise error. As stochastic methods, SGD and SAG-NUS has very fast convergence rate at the first several iterations, and keep fluctuating after convergence, while LBFGS has a relative slow convergence rate. For the performance of SAG-NUS, we believe that its strategy is too risky and sometimes makes too big step during update and stuck in local minimal. Overall, SGD is a good balance between convergence rate and accuracy.
