...
function run_exp() {
	...
	for p in ${p_values[@]};do
		echo "p = "$p
		$PETSC_DIR/$PETSC_ARCH/bin/mpirun -n 4 ../code_PETSc/seq_train -data ../data/train.txt -tdata ../data/test.txt -lambda $lambda -loss CRF -tol 1e-3 -tao_lmm_vectors $p > "p_"$p".txt"
	done
	mv p_*.txt ./output
}
...
function plot() {
	...
	for p in ${p_values[@]};do
		echo "p = "$p
		awk '{if($1 ~ /^[0-9]+$/ && NF == 9) print $0}'  "p_"$p".txt" > tmp$p.txt
	done
