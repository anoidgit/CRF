...
function plot() {
	...
	plot_setup="set terminal postscript color font ',30'; set style fill pattern border; set xlabel 'Effective number of passes'; set yrange [0:*]; set xrange [0:100];set ytics 0.2; set lmargin 5; set tmargin 0; set bmargin 1;set rmargin 1;"
	...
	echo $plot_setup" set ylabel 'Letter-wise error'; set output '| ps2pdf - t3_letter_err.pdf'; plot 'tmp1.txt' using 5:("'$8'"/100.0)  w l lw 2 title 'LBFGS', 'tmp2.txt' using 5:("'$8'"/100.0) w l lw 2 title 'SGD', 'tmp3.txt' using 5:("'$8'"/100.0) w l lw 2 title 'SAG-NUS';" | gnuplot
